...
export default function AcompanhamentoProcessos() {
  ...
  return <MobileLayout showCancelVisita showBack onBack={() => navigate('/desktop/iniciar-visita')} title="Acompanhamento de Processos">
    ...
    <div className="space-y-4 p-4 pb-32">
      ...
      {step === 3 && <EncerramentoVisita validateBeforeSave={validateBeforeSave} summaryItems={[{ label: 'Clientes vinculados', value: selectedClients.map((cliente) => cliente.nome).join(', ') || 'Nao informado' }, { label: 'Orgaos acompanhados', value: processo.orgao_ids.length }, { label: 'Processos acompanhados', value: processo.processo_ids.length }]} />}
    </div>
    {step < 3 && <MobileFooter><div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => setStep((value) => Math.max(0, value - 1))} disabled={step === 0}><ArrowLeft className="mr-2 h-4 w-4" />Voltar</Button><Button onClick={() => setStep((value) => Math.min(3, value + 1))}>Continuar<ArrowRight className="ml-2 h-4 w-4" /></Button></div></MobileFooter>}
  </MobileLayout>;
}
