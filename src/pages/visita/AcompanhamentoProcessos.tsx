...
const STEPS = ['Foto', 'Técnico', 'Identificação', 'Órgãos/processos', 'Registro', 'Radar', 'Final'];
...
export default function AcompanhamentoProcessos() {
  ...
  return <MobileLayout showCancelVisita showBack onBack={() => navigate('/desktop/iniciar-visita')} title="Acompanhamento de Processos">
    ...
    <div className="space-y-4 p-4 pb-32">
      ...
      {step === 3 && <RadarVisita />}
      {step === 4 && <EncerramentoVisita validateBeforeSave={validateBeforeSave} summaryItems={[{ label: 'Clientes vinculados', value: selectedClients.map((cliente) => cliente.nome).join(', ') || 'Nao informado' }, { label: 'Orgaos acompanhados', value: processo.orgao_ids.length }, { label: 'Processos acompanhados', value: processo.processo_ids.length }]} />}
    </div>
