...
export default function ClientesRapida() {
  useVisitRoute('/visita/rapida/clientes');
  ...
  const toggleCliente = (id: string) => {
    if (data.cliente_ids.includes(id)) removeClienteId(id);
  ...
  return (
    <MobileLayout showCancelVisita showBack onBack={() => navigate('/visita/rapida/tipos')} title="Cliente">
      ...
      <PageHeader
        icon={Users}
        title="Cliente"
        description="Selecione o cliente referente a essa visita"
      />
      ...
      <div className="px-4 pb-4 space-y-2">
        ...
        <Button variant="outline" onClick={openCreateDialog} className="w-full h-12 haptic-press">
      ...
      <div className="flex-1 overflow-auto scroll-smooth-y px-4 pb-4">
          ...
          <div className="mb-4 pb-4 border-b border-border">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedClientes.map((cliente) => (
                <SelectionChip key={cliente.id} selected onRemove={() => toggleCliente(cliente.id)}>
                  {cliente.nome}
                </SelectionChip>
              ))}
            </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            <EmptyState icon={Users} title="Nenhum cliente encontrado" description={`Nenhum resultado para "${search}"`} />
            <Button onClick={openCreateDialog} className="w-full h-12 haptic-press">
          ...
          <div className="space-y-2">
            {availableClientes.map((cliente) => (
              <SelectionCard key={cliente.id} onClick={() => toggleCliente(cliente.id)} showCheckbox>
                {cliente.nome}
              </SelectionCard>
            ))}
          </div>
      ...
      <MobileFooter>
        ...
        >
          {data.cliente_ids.length === 0 ? 'Selecione ao menos 1 cliente' : (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>O cliente será adicionado ao sistema e selecionado para esta visita.</DialogDescription>
          </DialogHeader>
          <div className="py-2">
            <Input
              ...
              autoFocus
              onKeyDown={(e) => { if (e.key === 'Enter') handleCreateCliente(); }}
            />
          </div>
          <DialogFooter className="gap-2 sm:gap-2">
            <Button variant="outline" onClick={() => setDialogOpen(false)} disabled={createCliente.isPending}>
              Cancelar
            </Button>
            <Button onClick={handleCreateCliente} disabled={createCliente.isPending || !novoNome.trim()}>
