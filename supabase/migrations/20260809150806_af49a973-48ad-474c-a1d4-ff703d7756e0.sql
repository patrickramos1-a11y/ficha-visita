...
alter table public.tipos_atendimento_config
  add column if not exists naturezas text[] not null default array['ATENDIMENTO'];
...
grant select, insert, update, delete on public.naturezas_visita to anon, authenticated;
grant all on public.naturezas_visita to service_role;
...
grant select, insert, update, delete on public.orgaos to anon, authenticated;
...
alter table public.processos_clientes
  add column if not exists situacao_atual text not null default 'AGUARDANDO_ANALISE';
grant select, insert, update, delete on public.processos_clientes to anon, authenticated;
grant all on public.processos_clientes to service_role;
...
grant select, insert, update, delete on public.atendimento_processos to anon, authenticated;
grant all on public.atendimento_processos to service_role;
...
grant select, insert, update, delete on public.mapeamentos_clientes_radar to anon, authenticated;
grant all on public.mapeamentos_clientes_radar to service_role;
...
grant select on public.integracao_radar_itens to anon, authenticated;
grant all on public.integracao_radar_itens to service_role;

create index if not exists atendimentos_natureza_data_idx
  on public.atendimentos (natureza, data_inicio desc);
create index if not exists processos_clientes_cliente_idx
  on public.processos_clientes (cliente_id);
create index if not exists integracao_radar_pendente_idx
  on public.integracao_radar_itens (status, atendimento_id);
